...
export function bindEvents({ camera, worldClientIso, errorHandler }) {
  return (node) => {
    const eventWorld = [
      L.props("clientX", "clientY"),
      L.pick({ x: "clientX", y: "clientY" }),
      L.inverse(worldClientIso),
      L.props("x", "y"),
    ];
    ...
    function touchesAngle(center, touches) {
      ...
      for (let t = touches.length - 1; t >= 0; t--) {
        const d = Math.atan2(
          touches[t].clientY - center.y,
          touches[t].clientX - center.x,
        );
        sum += d;
      }
    ...
    function onTouchStartLocal(evt) {
      ...
      touchBasePivotWorld = L.get(eventWorld, {
        clientX: touchBasePivot.x,
        clientY: touchBasePivot.y,
      });
    ...
    function onTouchMoveGlobal(evt) {
      if (evt.touches.length > 1 && localTouches.length > 0) {
        ...
        const dw =
          (Math.atan2(
            Math.sin(newAngle - touchBaseRot),
            Math.cos(newAngle - touchBaseRot),
          ) *
            180) /
          Math.PI;
    ...
    node.addEventListener("lostpointercapture", onPointerLostCapture, {
      capture: true,
    });
    node.addEventListener("getpointercapture", onPointerGotCapture, {
      capture: true,
    });
    ...
    if (nativeGestureEvents) {
      ...
      window.addEventListener("touchcancel", onTouchStopGlobal, {
        capture: true,
      });
    }
    ...
    return () => {
      if (nativeGestureEvents) {
        node.removeEventListener("gesturechange", onGestureStart, {
          capture: true,
        });
        node.removeEventListener("gesturestart", onGestureChange, {
          capture: true,
        });
        node.removeEventListener("gestureend", onGestureEnd, { capture: true });
      } else if (nativeTouchEvents) {
        window.removeEventListener("touchcancel", onTouchStopGlobal, {
          capture: true,
        });
        window.removeEventListener("touchend", onTouchStopGlobal, {
          capture: true,
        });
        ...
        node.removeEventListener("touchcancel", onTouchStopLocal, {
          capture: true,
        });
        node.removeEventListener("touchend", onTouchStopLocal, {
          capture: true,
        });
        node.removeEventListener("touchmove", onTouchMoveLocal, {
          capture: true,
        });
      ...
      node.removeEventListener("lostpointercapture", onPointerLostCapture, {
        capture: true,
      });
      node.removeEventListener("getpointercapture", onPointerGotCapture, {
        capture: true,
      });
