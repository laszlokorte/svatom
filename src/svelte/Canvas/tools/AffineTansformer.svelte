    ...
    <g
        ...
        onpointerdown={(evt) => {
            ...
            const pivot = evt.target.hasAttribute("data-pivot")
                ? JSON.parse(evt.target.getAttribute("data-pivot"))
                : null;
        ...
        onpointermove={(evt) => {
            ...
            if (
                ...
                const dxNew =
                    newPos.x - offset.value.x - activeScalePivot.value.x;
                const dyNew =
                    newPos.y - offset.value.y - activeScalePivot.value.y;
                const dxOld =
                    activeGrab.value.x -
                    offset.value.x -
                ...
                const dxNew =
                    newPos.x - offset.value.x - activeRotationPivot.value.x;
                const dyNew =
                    newPos.y - offset.value.y - activeRotationPivot.value.y;
                const dxOld =
                    activeGrab.value.x -
                    offset.value.x -
                ...
                const angle =
                    ((Math.atan2(dyNew, dxNew) - Math.atan2(dyOld, dxOld)) *
                        180) /
                    Math.PI;
        ...
        <g transform={rotationTransform.value}>
            ...
            >
                {#if selectionExtensionValue.allowedTransform.rotate}
                    <line
                        x1={(selectionExtensionValue.minXPadded -
                            ...
                            padding) /
                            2}
                        y1={selectionExtensionValue.minYPadded - padding}
                        ...
                        y2={selectionExtensionValue.minYPadded -
                            padding -
                            40 * Math.min(cameraScaleValue, 2)}
                        stroke="RoyalBlue"
                ...
                {#if R.any(R.identity, R.values(selectionExtensionValue.allowedTransform))}
                    <rect
                        data-translate="true"
                        x={selectionExtensionValue.minXPadded - padding}
                        y={selectionExtensionValue.minYPadded - padding}
                        width={selectionExtensionValue.maxXPadded +
                            padding -
                            (selectionExtensionValue.minXPadded - padding)}
                        height={selectionExtensionValue.maxYPadded +
                            padding -
                            (selectionExtensionValue.minYPadded - padding)}
                        class="box"
                        pointer-events="all"
                        fill="none"
                        tabindex="-1"
                ...
                {#each handles as handle, i (i)}
                    ...
                    {@const cxpadded = U.lerp(
                        selectionExtensionValue.minXPadded - padding,
                        selectionExtensionValue.maxXPadded + padding,
                        (handle.wx + 1) / 2,
                    )}
                    {@const cypadded = U.lerp(
                        selectionExtensionValue.minYPadded - padding,
                        selectionExtensionValue.maxYPadded + padding,
                        (handle.wy + 1) / 2,
                    )}
                    {@const px = U.lerp(
                        selectionExtensionValue.minX,
                    ...
                    {#if R.all((t) => selectionExtensionValue.allowedTransform[t], handle.transforms)}
                        <g
                            class={{
                                active:
                                    activeHandle.value &&
                                    activeHandle.value.wx ===
                                        handle.wx *
                                            Math.sign(scaleAccum.value.x) &&
                                    activeHandle.value.wy ==
                                        handle.wy *
                                            Math.sign(scaleAccum.value.y),
                            }}
                        >
                            <circle
                                data-handle={i}
