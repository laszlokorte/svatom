...
export const marchingCubesToGeo = (fn, minX, minY, minZ, maxX, maxY, maxZ, res = 4) => {
  ...
  function getEdgeKey(x, y, z, edgeIndex) {
    return `${x},${y},${z},${edgeIndex}`;
  }
  ...
  function getOrCreateEdge(a, b) {
  	...
  	if(edgeCache.has(key)) {
  		return edges[edgeCache.get(key)]
  	} else {
  		const e = { vertices: k, faces: [],  attrs: { class: "obj-edge", color: "#333", flip: false } }
  ...
  function getOrCreateVertex(x, y, z, edgeIndex, cubeCorners, cubeValues) {
    const key = getEdgeKey(x, y, z, edgeIndex);
    if (vertexCache.has(key)) return vertexCache.get(key);

    const [c1, c2] = edgeVertexIndices[edgeIndex];
    const p1 = cubeCorners[c1];
    const p2 = cubeCorners[c2];
    const valp1 = cubeValues[c1];
    const valp2 = cubeValues[c2];

    const vert = vertexInterp(0, p1, p2, valp1, valp2);
    const idx = vertices.push(vert) - 1;
    vertexCache.set(key, idx);
    return idx;
  }

  // Sample grid step sizes
  ...
  for (let x = 0; x < res; x++) {
    for (let y = 0; y < res; y++) {
      for (let z = 0; z < res; z++) {
        ...
        // Build cube corners and their scalar values
        const cubeCorners = [];
        const cubeValues = [];
        for (let i = 0; i < 8; i++) {
          const ix = x + (i & 1);
          ...
          const py = minY + iy * dy;
          const pz = minZ + iz * dz;
          cubeCorners[i] = {x: px, y: py, z: pz};
          cubeValues[i] = fn(px, py, pz);
        }
        ...
        for (let i = 0; i < 12; i++) {
          if (edgeTable[cubeIndex] & (1 << i)) {
            vertList[i] = getOrCreateVertex(x, y, z, i, cubeCorners, cubeValues);
          }
        }
        ...
        for (let i = 0; tri[i] !== -1; i += 3) {
          const ia = vertList[tri[i]];
          const ib = vertList[tri[i + 1]];
          const ic = vertList[tri[i + 2]];
          const f = faces.push({vertices: [ic, ib, ia], attrs: {class: "obj-face"}}) - 1;

          const e1 = getOrCreateEdge(ia, ib)
          const e2 = getOrCreateEdge(ib, ic)
          const e3 = getOrCreateEdge(ia, ic)
          e1.faces.push(f)
          e2.faces.push(f)
          e3.faces.push(f)
